...
class TaskWordAppendTool(BaseTool):
    name = "task_word_append"
    description = "Append text to a Word document inside Ubuntu with structured verification and a verified output artifact."
    input_schema = {
        "type": "object",
        "properties": {
            "source": path_ref_schema(description="Source Word document path."),
            "text": {
                "type": "string",
    ...
    def execute(self, arguments: dict[str, Any]) -> ToolResult:
        ...
        append_text = arguments.get("text")
        if not isinstance(append_text, str) or not append_text.strip():
            self.set_error_type("tool_error")
            raise ValueError("task_word_append.text must be a non-empty string")
        ...
        if source_ref.layer == "host" and not Path(source_ref.path).expanduser().resolve().is_file():
            self.set_error_type("tool_error")
            raise ValueError(f"source docx does not exist: {source_ref.path}")

        suffix = unique_suffix()
        work_dir = ubuntu_path("word_append", on_device=self._on_device)
        remote_output = ubuntu_path("word_append", f"output-{suffix}.docx", on_device=self._on_device)
        remote_input = ubuntu_path("word_append", f"input-{suffix}{Path(source_ref.path).suffix or '.docx'}", on_device=self._on_device)
        ...
        try:
            ...
            if not ensure_result.ok:
                ...
                return ToolResult(
                    ...
                    summary="task_word_append failed while preparing the Ubuntu work directory",
        ...
        input_paragraphs = payload.get("input_paragraphs") if isinstance(payload.get("input_paragraphs"), int) else None
        output_paragraphs = payload.get("output_paragraphs") if isinstance(payload.get("output_paragraphs"), int) else None
        last_paragraph = payload.get("last_paragraph", "") if isinstance(payload, dict) else ""
        file_exists = bool(payload.get("file_exists")) if isinstance(payload, dict) else False
        checks = {
            "source_ready": source_ready,
            "python_ok": python_result.ok,
            "output_exists": output_stat.ok and file_exists,
            "paragraph_count_incremented": input_paragraphs is not None and output_paragraphs == input_paragraphs + 1,
            "append_text_matches": last_paragraph == append_text,
        }
        ...
        return ToolResult(
            ...
            output=summarize_backend_steps(steps),
            summary=(
                f"task_word_append {'succeeded' if ok else 'failed'} with append_text_matches={checks['append_text_matches']}"
            ),
    ...
    def _prepare_source(
        ...
        *,
        ...
        stage_relative = termux_stage_relative("task-word-append", Path(source_ref.path).name)
    ...
    def _build_code(self, *, input_path: str, output_path: str, append_text: str) -> str:
        return f"""
import json
from pathlib import Path
...
append_text = {json.dumps(append_text)}

input_doc = Document(input_path)
input_paragraphs = len(input_doc.paragraphs)
input_doc.add_paragraph(append_text)
input_doc.save(output_path)
output_doc = Document(output_path)
last_paragraph = output_doc.paragraphs[-1].text if output_doc.paragraphs else ''
print(json.dumps({{
...
""".strip()
    ...
    def _parse_timeout(self, timeout_value: Any) -> float | None:
        ...
        if not isinstance(timeout_value, (int, float)):
            self.set_error_type("tool_error")
            raise ValueError("task_word_append.timeout_seconds must be numeric when provided")
